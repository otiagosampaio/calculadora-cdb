...
with col_logo:
    st.image("https://i.imgur.com/1bF9g1k.png", width=120)  # Logo oficial da Traders
with col_titulo:
    st.markdown("<h1 style='margin-top: 20px; color: #6B48FF;'>traders</h1>", unsafe_allow_html=True)
    st.markdown("<h2 style='color: #333;'>Calculadora de CDB Pré e Pós-fixado</h2>", unsafe_allow_html=True)
...
with st.expander("Parâmetros do Investimento", expanded=True):
    ...
    if tipo_cdb == "Pós-fixado (% do CDI)":
        ...
        dias_ano = 252
        st.info(f"Taxa efetiva contratada: **{taxa_anual:.2f}% a.a.** ({perc_cdi}% do CDI)")
    else:
...
base_ir = rendimento_apos_iof
valor_ir = base_ir * (aliquota_ir / 100)
montante_liquido = investimento + base_ir - valor_ir
...
with c2:
    if considerar_iof and prazo_dias < 30:
        st.write(f"**IOF aplicado:** {(1-aliquota_iof)*100:.0f}%")
    st.write(f"**Alíquota de IR:** {aliquota_ir}%")
...
st.markdown(
    ...
    f"Simulação elaborada por <b>{nome_assessor}</b> em {data_simulacao.strftime('%d/%m/%Y')} para <b>{nome_cliente}</b><br>"
    f"<b>traders Corretora</b> • Assessoria de Investimentos</p>",
